...
def read_and_classify_test_set(filename, probs_classes, class_labels):
    ...
    for line in file_test_set:
        ...
        print "yi: expected class %d, assigend class %d.  OK? %s" % (expected_class, assigned_class, (expected_class == assigned_class))
...
filenames = ['breast-cancer-wisconsin.data.train.benign.scaled_log_prob', 'breast-cancer-wisconsin.data.train.malignant.scaled_log_prob']

probs_classes = read_log_probabilities(filenames)

read_and_classify_test_set('breast-cancer-wisconsin.data.test', probs_classes, class_labels)
